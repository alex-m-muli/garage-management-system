...
// --- CRITICAL FIX 1: Define the Base URL for the Deployed Backend ---
...
  const fetchServices = () => {
    // FIX 1A: Use the full base URL for GET request
...
      .catch((err) => {
        ...
        // If fetch fails, ensure services is reset to an empty array to prevent map crash
        setServices([]); 
      });
  };
...
    e.preventDefault();
    // FIX 1B: Use the full base URL for POST/PUT requests
    const url = editId ? `${API_BASE_URL}/api/services/${editId}` : `${API_BASE_URL}/api/services`;
    
...
    }).catch((err) => {
        console.error("Error saving service:", err.response ? err.response.data : err.message);
...
      // FIX 1C: Use the full base URL for DELETE request
...
      {/* === Services Grid === */}
      <Grid>
        {/* FIX 2: Defensive check if 'services' is an array before calling .map() */}
